...
export default function ProductInfo({ product }) {
    ...
    const sizes = useMemo(() => [...new Set(variants.map((item) => item.size).filter(Boolean))], [variants]);

    const colors = useMemo(() => [...new Set(variants.map((item) => item.color).filter(Boolean))], [variants]);
    ...
    return (
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            ...
            {sizes.length > 0 && (
                <OptionGroup title="Size" items={sizes} selected={selectedSize} onSelect={setSelectedSize} />
            )}

            {colors.length > 0 && (
                <OptionGroup title="Màu sắc" items={colors} selected={selectedColor} onSelect={setSelectedColor} />
            )}
...
function OptionGroup({ title, items, selected, onSelect }) {
    return (
        <div className="mt-5">
