...
function mapHomeCampaign(campaign = {}) {
    return {
        ...
        banner: normalizeImage(campaign.banner),
        thumbnail: normalizeImage(campaign.thumbnail),
        image: normalizeImage(campaign.thumbnail),

        limit: Number(campaign.limit || 0),
        ...
        countdownSeconds: Number(campaign.countdown_seconds || 0),
        time: getCountdownParts(campaign.countdown_seconds),
...
export function mapHomeResponse(response = {}) {
    return {
        ...
        newProducts: Array.isArray(response.new_products)
            ? response.new_products.map(mapHomeProduct)
            : [],
        ...
        activeCampaigns: Array.isArray(response.active_campaigns)
            ? response.active_campaigns.map(mapHomeCampaign)
            : [],
        ...
        categories: Array.isArray(response.categories)
            ? response.categories.map(mapHomeCategory)
            : [],

        newsEvents: Array.isArray(response.news_events)
            ? response.news_events.map(mapHomeNews)
            : [],

        trendingKeywords: Array.isArray(response.trending_keywords)
