...
export default function Checkout() {
    ...
    const checkoutItems =
        selectedIds.length === 0 ? cartItems : cartItems.filter((item) => selectedIds.includes(item.cartItemId));
    ...
    const checkoutTotalItems = checkoutItems.reduce((total, item) => total + Number(item.quantity || 0), 0);

    function validate() {
        const nextErrors = {};

        if (!pendingPaymentOrder && cartItems.length === 0) {
