...
function mapReceiver(receiver = {}) {
    return { name: receiver.name || '', phone: receiver.phone || '', address: receiver.address || '', raw: receiver };
}
...
function mapOrderItem(item = {}) {
    ...
    return {
        id: item.id, productId, productVariantId,
        ...
        price: finalPrice, originalPrice, discountAmount, finalPrice,
        quantity: toNumber(item.quantity),
        total: toNumber(item.total || finalPrice * toNumber(item.quantity)),
        promotion: item.promotion || item.promotion_snapshot || null,
        reviewId, reviewed: Boolean(item.is_reviewed || item.reviewed || reviewId), canReview: Boolean(item.can_review),
        raw: item,
    };
}

function mapPickup(pickup = {}) {
    return { location: pickup.location || '', instruction: pickup.instruction || '', raw: pickup };
}

function mapTimelineItem(item = {}) {
    return { key: item.key || '', label: item.label || '', done: Boolean(item.status), time: item.time || '', note: item.note || '', raw: item };
}

function mapOrderDetail(item = {}) {
    const fulfillmentMethod = item.fulfillment_method || '';
    const statusText = item.status === 'awaiting_receipt' ? (fulfillmentMethod === 'pickup' ? 'Sẵn sàng nhận tại phòng' : 'Đang giao') : orderStatusText(item.status);
    return {
        id: item.id,
        code: item.order_code || '',
        orderCode: item.order_code || '',
        ...
        receiver: mapReceiver(item.receiver || { name: item.shipping_name, phone: item.shipping_phone, address: item.shipping_address }),
        payment: item.payment ? mapPayment(item.payment) : { status: item.payment_status || '', statusText: paymentStatusText(item.payment_status), method: item.payment_method || '', methodText: paymentMethodText(item.payment_method) },
        actions: mapActions(item.actions),
        summary: mapSummary(item.summary || { sub_total: item.sub_total || item.total, shipping_fee: item.shipping_fee, discount: item.discount_total, grand_total: item.grand_total || item.total }),
...
export function mapOrderDetailResponse(response = {}) {
    return { success: Boolean(response.success), message: response.message || '', ...mapOrderDetail(response.data || {}) };
}
...
export function mapGuestOrderLookupResponse(response = {}) {
    ...
    return {
        ...
        orders: lookupType === 'phone_email' ? (Array.isArray(item.orders) ? item.orders : []).map((order) => mapOrderDetail(order)) : [],
        raw: item,
    };
}
...
export function mapOrderListResponse(response = {}) {
    ...
    return {
        ...
        meta: { currentPage: toNumber(response.meta?.current_page || 1), lastPage: toNumber(response.meta?.last_page || 1), perPage: toNumber(response.meta?.per_page || 10), total: toNumber(response.meta?.total || raw.length) },
        raw: response,
    };
}

function paymentStatusText(status) {
    const map = { unpaid: 'Chưa thanh toán', paid: 'Đã thanh toán', failed: 'Thanh toán thất bại', refunded: 'Đã hoàn tiền' };
    return map[status] || status || 'Chưa tạo thanh toán';
}

function paymentMethodText(method) {
    const map = { cod: 'Thanh toán khi nhận hàng', mock_bank: 'Chuyển khoản ngân hàng', cash_on_pickup: 'Thanh toán trực tiếp khi nhận tại Phòng Công tác Chính trị & Quản lý sinh viên Trường Đại học Kỹ thuật - Công nghệ Cần Thơ' };
